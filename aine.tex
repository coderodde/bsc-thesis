...
\label{Dijkstra}
\end{finalgo}
\noindent $A\ast$:n pseudokoodi on tasan sama kuin Dijkstran algoritmin, paitsi että rivillä 3 $g(x)$:n sijasta on $f(x)$, jolle siis $f(x) = g(x) + h(x)$. Molemmat kutsuvat \textsc{Traceback-Path}-rutiinin, joka siis muodostaa lyhimmän polun ''edeltäjäpuusta'' (engl. \textit{predecessor tree}) ajassa $\Theta(N)$, missä $N$ on lyhimmän polun solmujen määrä. On huomattava, että $A\ast$ palautuu Dijkstran algoritmiin määrittelemällä $h(u) = 0$ jokaisella $u \in V$.
\begin{finalgo}
$u = x$ \\
$p = \langle  \rangle$ \\
...
\]
ja korvaamalla rivin 7 kutsu kutsulla rutiiniin $\textsc{Terminate}\ast(\dots)$, jonka määritelmä on alla.
\begin{finalgo}
