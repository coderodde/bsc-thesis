...
$x_b = \underset{x \in \text{OPEN}_{REV}}{\argmin g_{REV}(x)} $ \\ 
...
\begin{finalgo}[h]
$x_f = \underset{x \in \text{OPEN}}{\argmin f(x)}$ \\
$x_b = \underset{x \in \text{OPEN}_{REV}}{\argmin f_{REV}(x)}$ \\
\If{$\min(f(x_f), f_{REV}(x_b)) \leq \mu$}{
  \KwRet \textsc{Traceback-Path}$(m, \pi, \pi_{REV})$\\
}
\KwRet $\textbf{nil}$ \\
